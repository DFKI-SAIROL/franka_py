...
private:
    struct CameraSet {
        ...
        bool ready() const { return rgb && depth && conf; }
    };
    ...
    void ingest(sensor_msgs::msg::Image::ConstSharedPtr msg, std::string name, int type) {
        ...
        if (found) {
            ...
            for (auto& n : cam_names_) {
                ...
                auto img_c = *cam_data.conf; img_c.header.stamp = master_stamp;
    ...
    void print_stats_if_needed() {
        if (stats_ingested_ % 300 == 0) {
            ...
            if (stats_ingested_ > 0) success_rate = (double)stats_matches_ / (stats_ingested_ / 9.0) * 100.0; 
