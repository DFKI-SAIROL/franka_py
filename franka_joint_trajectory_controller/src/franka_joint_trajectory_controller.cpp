...
#include "franka_joint_trajectory_controller/franka_joint_trajectory_controller.hpp"
...
#include "franka_joint_trajectory_controller/trajectory.hpp"
...
namespace franka_joint_trajectory_controller
{
...
controller_interface::CallbackReturn FrankaJointTrajectoryController::on_init()
{
  try
  {
...
FrankaJointTrajectoryController::command_interface_configuration() const
{
  ...
  for (const auto & joint_name : command_joint_names_)
  {
    ...
    {
      conf.names.push_back(joint_prefix_ + joint_name + "/" + interface_type);
    }
  }
  return conf;
}
...
controller_interface::InterfaceConfiguration
FrankaJointTrajectoryController::state_interface_configuration() const
{
  controller_interface::InterfaceConfiguration conf;
  conf.type = controller_interface::interface_configuration_type::INDIVIDUAL;
...
controller_interface::return_type FrankaJointTrajectoryController::update(
...
void FrankaJointTrajectoryController::read_state_from_state_interfaces(JointTrajectoryPoint & state)
{
...
bool FrankaJointTrajectoryController::read_state_from_command_interfaces(JointTrajectoryPoint & state)
{
...
bool FrankaJointTrajectoryController::read_commands_from_command_interfaces(
...
void FrankaJointTrajectoryController::query_state_service(
...
controller_interface::CallbackReturn FrankaJointTrajectoryController::on_configure(
  const rclcpp_lifecycle::State &)
{
  ...
  joint_prefix_ = params_.arm_prefix;
  if(joint_prefix_ != "") 
  {
    joint_prefix_ += "_";
  }
  ...
  joint_command_subscriber_ =
    get_node()->create_subscription<trajectory_msgs::msg::JointTrajectory>(
      ...
      std::bind(&FrankaJointTrajectoryController::topic_callback, this, std::placeholders::_1));
  ...
  action_server_ = rclcpp_action::create_server<FollowJTrajAction>(
    ...
    std::bind(&FrankaJointTrajectoryController::goal_received_callback, this, _1, _2),
    std::bind(&FrankaJointTrajectoryController::goal_cancelled_callback, this, _1),
    std::bind(&FrankaJointTrajectoryController::goal_accepted_callback, this, _1));
  ...
  query_state_srv_ = get_node()->create_service<control_msgs::srv::QueryTrajectoryState>(
    ...
    std::bind(&FrankaJointTrajectoryController::query_state_service, this, _1, _2));

  return CallbackReturn::SUCCESS;
}

controller_interface::CallbackReturn FrankaJointTrajectoryController::on_activate(
  const rclcpp_lifecycle::State &)
{
  auto logger = get_node()->get_logger();
  ...
  for (const auto & interface : params_.command_interfaces)
  {
    ...
    if (!franka_controller_interface::get_ordered_interfaces(
          command_interfaces_, command_joint_names_, interface, joint_command_interface_[index], joint_prefix_))
    {
      RCLCPP_ERROR(
  ...
  for (const auto & interface : params_.state_interfaces)
  {
    ...
    auto index = static_cast<size_t>(std::distance(allowed_interface_types_.begin(), it));
    if (!franka_controller_interface::get_ordered_interfaces(
          state_interfaces_, params_.joints, interface, joint_state_interface_[index], joint_prefix_))
    {
      RCLCPP_ERROR(
...
controller_interface::CallbackReturn FrankaJointTrajectoryController::on_deactivate(
  const rclcpp_lifecycle::State &)
{
...
controller_interface::CallbackReturn FrankaJointTrajectoryController::on_cleanup(
  const rclcpp_lifecycle::State &)
{
  return CallbackReturn::SUCCESS;
}

controller_interface::CallbackReturn FrankaJointTrajectoryController::on_error(
  const rclcpp_lifecycle::State &)
{
  if (!reset())
  {
...
bool FrankaJointTrajectoryController::reset()
{
  subscriber_is_active_ = false;
...
void FrankaJointTrajectoryController::publish_state(
...
void FrankaJointTrajectoryController::topic_callback(
...
rclcpp_action::GoalResponse FrankaJointTrajectoryController::goal_received_callback(
...
rclcpp_action::CancelResponse FrankaJointTrajectoryController::goal_cancelled_callback(
...
void FrankaJointTrajectoryController::goal_accepted_callback(
...
void FrankaJointTrajectoryController::fill_partial_goal(
...
void FrankaJointTrajectoryController::sort_to_local_joint_order(
...
bool FrankaJointTrajectoryController::validate_trajectory_point_field(
...
bool FrankaJointTrajectoryController::validate_trajectory_msg(
...
void FrankaJointTrajectoryController::add_new_trajectory_msg(
  const std::shared_ptr<trajectory_msgs::msg::JointTrajectory> & traj_msg)
{
  traj_msg_external_point_ptr_.writeFromNonRT(traj_msg);
}

void FrankaJointTrajectoryController::preempt_active_goal()
{
  const auto active_goal = *rt_active_goal_.readFromNonRT();
  if (active_goal)
  {
...
std::shared_ptr<trajectory_msgs::msg::JointTrajectory>
FrankaJointTrajectoryController::set_hold_position()
{
...
std::shared_ptr<trajectory_msgs::msg::JointTrajectory>
FrankaJointTrajectoryController::set_success_trajectory_point()
{
...
bool FrankaJointTrajectoryController::contains_interface_type(
...
void FrankaJointTrajectoryController::resize_joint_trajectory_point(
...
void FrankaJointTrajectoryController::resize_joint_trajectory_point_command(
  trajectory_msgs::msg::JointTrajectoryPoint & point, size_t size)
{
  if (has_position_command_interface_)
  {
...
bool FrankaJointTrajectoryController::has_active_trajectory() const
{
  return traj_external_point_ptr_ != nullptr && traj_external_point_ptr_->has_trajectory_msg();
}

void FrankaJointTrajectoryController::update_pids()
{
  for (size_t i = 0; i < dof_; ++i)
  {
    const auto & gains = params_.gains.joints_map.at(params_.joints[i]);
...
void FrankaJointTrajectoryController::init_hold_position_msg()
{
...
PLUGINLIB_EXPORT_CLASS(
  franka_joint_trajectory_controller::FrankaJointTrajectoryController, controller_interface::ControllerInterface)
