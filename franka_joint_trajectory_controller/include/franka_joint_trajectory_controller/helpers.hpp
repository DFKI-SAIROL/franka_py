...
namespace franka_controller_interface
{
...
bool get_ordered_interfaces(
  ...
  const std::string & interface_type, std::vector<std::reference_wrapper<T>> & ordered_interfaces, std::string &prefix)
{
  ...
  for (const auto & name : ordered_names)
  {
    for (auto & interface : unordered_interfaces)
    {
      if (!interface_type.empty())
      {
        ...
        // std::cout << prefix << name << " == " << interface.get_prefix_name() << " && " <<
        //        interface_type << " == " << interface.get_interface_name() << " || " << 
        //        prefix << name << " / " << interface_type << " == " << interface.get_name() << std::endl;

        if (
          ((prefix + name == interface.get_prefix_name()) &&
           (interface_type == interface.get_interface_name())) ||
          ((prefix + name + "/" + interface_type) == interface.get_name()))
        {
